	...
	public function tab_shortcode($atts, $content = null) {
		...
		if ( is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX || (!empty($ajax_action) && $ajax_action == 'load_template' || $ajax_action == 'components_modal')) ) {
			...
			switch ($fsn_tab_output_phase) {
				case 'tab_nav':
					$output .= '<li'. ($fsn_tab_counter === 1 ? ' class="active"' : '') .'><a href="#'. esc_attr($tab_id) .'" role="button" data-bs-target="#'. esc_attr($tab_id) .'" data-bs-toggle="tab" class="'.($fsn_tab_counter === 1 ? 'active' : '').''. (!empty($classes) ? ' '. esc_attr($classes) .'' : '') .'">'. esc_html($tab_title) .'</a></li>';
					break;
				case 'tab_content':
