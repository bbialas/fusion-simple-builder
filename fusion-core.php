 ...
 */
/**
 * Plugin Name: Fusion Builder - JF fork
...
class FusionCore	{
	...
	public function admin_enqueue_scripts_styles($hook_suffix) {
		...
		if ( ($hook_suffix == 'post.php' || $hook_suffix == 'post-new.php') && ( (post_type_exists('notification') && $post->post_type == 'notification') || (!empty($fsn_post_types) && is_array($fsn_post_types) && in_array($post->post_type, $fsn_post_types)) ) ) {
			...
			//plugin
			wp_enqueue_script( 'fsn_base64', plugin_dir_url( __FILE__ ) . 'includes/js/base64.min.js', [], '1.1.0', true );
	...
	public function init_style_global() {
		if (is_admin()) {
			...
			$fsn_style_params = array(
				...
				array(
					'type' => 'text',
					'param_name' => 'user_row_classes',
					'label' => __('CSS Row Classes', 'fusion'),
				...
				array(
					'type' => 'text',
					'param_name' => 'user_col_classes',
					'label' => __('CSS Col Classes', 'fusion'),
					'help' => __('Separate multiple classes with a space.', 'fusion'),
					'section' => 'style'
				)
			);
	...
	public function row_shortcode($atts, $content = null) {
		...
		if ( is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX || (!empty($ajax_action) && $ajax_action == 'load_template' || $ajax_action == 'components_modal')) ) {
			...
			if (empty($atts['user_row_classes'])) {
				$rowCssClasses = 'row';
			} else {
				$rowCssClasses = esc_attr($atts['user_row_classes']);
			}

			$output .= '<div class="'.$rowCssClasses.'">'. do_shortcode($content) .'</div>';
	...
	public function column_shortcode($atts, $content = null) {
		...
		if ( is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX || (!empty($ajax_action) && $ajax_action == 'load_template' || $ajax_action == 'components_modal')) ) {
			...
			if (empty($atts['user_col_classes'])) {
				$colCssClasses = 'col-sm-'. esc_attr($width) . (!empty($offset) ? ' col-sm-offset-'. esc_attr($offset) : '');
			} else {
				$colCssClasses = esc_attr($atts['user_col_classes']);
			}

			$output .= '<div class="'.$colCssClasses.'"><div class="fsn-column-inner '. fsn_style_params_class($atts) . (!empty($classes) ? ' '. esc_attr($classes) : '') .'"'. (!empty($style) ? ' style="'. esc_attr($style) .'"' : '') .'>'. do_shortcode($content) .'</div></div>';
	...
	public static function get_input_field($param, $param_value = '') {
		...
		switch($param['type']) {
			...
			case 'textarea':
				...
				$input .= '<script>';
				$input .= 'var codeTextArea = document.getElementById("fsn_code");';
				$input .= 'setTimeout(function() { codeTextArea.focus();';
				$input .= 'var myCodeMirror = CodeMirror.fromTextArea(codeTextArea, {lineNumbers: true, autofocus: true});';
				$input .= 'myCodeMirror.on("change", function() { myCodeMirror.save(); });';
				$input .= ' }, 1);';
				$input .= '</script>';
				break;
			case 'checkbox':
